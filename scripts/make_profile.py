...
def make_readme():
    header_text: List[str] = open('profile/header.md', 'r').readlines()
    footer_text: List[str] = open('profile/footer.md', 'r').readlines()
    members: Dict[str, List[Dict[str, str]]] = yaml.load(open('profile/members.yaml', 'r'), Loader=yaml.FullLoader)

    member_text: List[str] = make_member_table(members['member'])

    blanks: List[str] = ['\n\n']
    all_text: List[str] = header_text + blanks + member_text + blanks + footer_text
